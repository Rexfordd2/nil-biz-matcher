...
- Add better error messaging for business search (e.g., distinguish 401 unauthorized vs 429 rate limit).
- Validate numeric inputs (followers, hours/week) to prevent `NaN` on malformed input.
- Add `.env.example` and README section for `GOOGLE_MAPS_API_KEY`, `GOOGLE_MAPS_REGION_BIAS`.
